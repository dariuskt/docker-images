...
init_use_startup_trigger() {
	...
	then
		...
		local ST=$(echo "${USE_STARTUP_TRIGGER}" | tr '[:upper:]' '[:lower:]')
	fi
...
init_wait_for_a_not_empty_dir() {
	...
	then
		...
		local DR="${WAIT_FOR_A_NOT_EMPTY_DIR}"
	fi
...
init_wait_for_a_not_empty_file() {
	...
	then
		...
		local DR="${WAIT_FOR_A_NOT_EMPTY_FILE}"
	fi
